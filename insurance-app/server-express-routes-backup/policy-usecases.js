...
function bmf(from, to, alias = 'bm', startIdx = 1) {
  const c = [], p = []; let i = startIdx;
...
// =====================================================================
router.get('/amendments/kpis', async (req, res) => {
  ...
  try {
    const sql = `
      ...
      WHERE 1=1 ${f.w};
    `;
    const { rows } = await pool.query(sql, f.p);
    res.json(rows[0] || {});
  } catch (err) { res.status(500).json({ error: err.message }); }
});

router.get('/amendments/by-event-type', async (req, res) => {
  const { from, to } = req.query;
  const f = bmf(from, to);
  try {
    const sql = `
      SELECT
        COALESCE(fpt.event_type_code,'UNKNOWN') AS event_type,
        COUNT(*) AS txn_count,
...
router.get('/amendments/monthly', async (req, res) => {
  const { from, to } = req.query;
  const f = bmf(from, to);
  try {
    const sql = `
...
router.get('/amendments/by-lob', async (req, res) => {
  const { from, to } = req.query;
  const f = bmf(from, to);
  try {
    const sql = `
...
// =====================================================================
router.get('/concentration/kpis', async (req, res) => {
  const { from, to } = req.query;
  const f = bmf(from, to);
  try {
    const sql = `
...
router.get('/concentration/by-region', async (req, res) => {
  const { from, to } = req.query;
  const f = bmf(from, to);
  try {
    const sql = `
      ...
      WHERE 1=1 ${f.w}
      GROUP BY region ORDER BY written DESC LIMIT 10;
    `;
    const { rows } = await pool.query(sql, f.p);
    res.json(rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
...
router.get('/concentration/by-lob', async (req, res) => {
  const { from, to } = req.query;
  const f = bmf(from, to);
  try {
    const sql = `
...
router.get('/concentration/by-territory', async (req, res) => {
  const { from, to } = req.query;
  const f = bmf(from, to);
  try {
    const sql = `
      ...
      WHERE 1=1 ${f.w}
      GROUP BY territory ORDER BY written DESC LIMIT 10;
    `;
    const { rows } = await pool.query(sql, f.p);
    res.json(rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
...
// =====================================================================
router.get('/special-flags/kpis', async (req, res) => {
  const { from, to } = req.query;
  const f = bmf(from, to);
  try {
    const sql = `
...
router.get('/special-flags/by-flag', async (req, res) => {
  const { from, to } = req.query;
  const f = bmf(from, to);
  try {
    const sql = `
    ...
    `;
    // Each subquery uses same $1/$2 params
    const { rows } = await pool.query(sql, f.p);
    res.json(rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
});

router.get('/special-flags/by-lob', async (req, res) => {
  const { from, to } = req.query;
  const f = bmf(from, to);
  try {
    const sql = `
...
// =====================================================================
router.get('/vintage/kpis', async (req, res) => {
  const { from, to } = req.query;
  const f1 = bmf(from, to, 'bm', 1);
  const f2 = bmf(from, to, 'bm', f1.n);
  try {
    const sql = `
      WITH pol AS (
        SELECT fpt.policy_key,
               EXTRACT(YEAR FROM dp.original_inception_date)::int AS inception_year,
    ...
    `;
    const { rows } = await pool.query(sql, [...f1.p, ...f2.p]);
    res.json(rows[0] || {});
  } catch (err) { res.status(500).json({ error: err.message }); }
});

router.get('/vintage/by-cohort', async (req, res) => {
  const { from, to } = req.query;
  const f1 = bmf(from, to, 'bm', 1);
  const f2 = bmf(from, to, 'bm', f1.n);
  try {
    const sql = `
      WITH pol AS (
        SELECT fpt.policy_key,
               COALESCE(EXTRACT(YEAR FROM dp.original_inception_date)::int, bm.book_year) AS cohort,
               fpt.direct_written_premium, fpt.direct_earned_premium,
               fpt.new_or_renewal_code
        FROM reporting.fact_policy_transaction fpt
        JOIN reporting.dim_book_month bm ON bm.book_month_key = fpt.book_month_key
...
router.get('/vintage/nb-vs-renewal', async (req, res) => {
  const { from, to } = req.query;
  const f1 = bmf(from, to, 'bm', 1);
  const f2 = bmf(from, to, 'bm', f1.n);
  try {
...
// =====================================================================
router.get('/term-recon/kpis', async (req, res) => {
  const { from, to } = req.query;
  const f = bmf(from, to);
  try {
    const sql = `
...
router.get('/term-recon/by-lob', async (req, res) => {
  const { from, to } = req.query;
  const f = bmf(from, to);
  try {
    const sql = `
...
router.get('/term-recon/by-product', async (req, res) => {
  const { from, to } = req.query;
  const f = bmf(from, to);
  try {
    const sql = `
...
router.get('/term-recon/yearly', async (req, res) => {
  const { from, to } = req.query;
  const f = bmf(from, to);
  try {
    const sql = `
...
// =====================================================================
router.get('/peril-audit/kpis', async (req, res) => {
  const { from, to } = req.query;
  const f = bmf(from, to);
  try {
    const sql = `
...
router.get('/peril-audit/by-lob', async (req, res) => {
  const { from, to } = req.query;
  const f = bmf(from, to);
  try {
    const sql = `
...
router.get('/peril-audit/by-coverage', async (req, res) => {
  const { from, to } = req.query;
  const f = bmf(from, to);
  try {
    const sql = `
...
router.get('/peril-audit/form-detail', async (req, res) => {
  const { from, to } = req.query;
  const f = bmf(from, to);
  try {
    const sql = `
