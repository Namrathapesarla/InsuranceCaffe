...
/* ------------------------------------------------------------------ */
/*  Helper: date-range WHERE clause for book_month dimension          */
/* ------------------------------------------------------------------ */
function bookMonthFilter(from, to, alias = 'bm', startIdx = 1) {
  const clauses = [];
  const params = [];
  let idx = startIdx;
...
router.get('/kpis', async (req, res) => {
  ...
  const f1 = bookMonthFilter(from, to, 'bm', 1);
  const f2 = bookMonthFilter(from, to, 'bm', f1.nextIdx);
  try {
    const sql = `
      WITH policy_base AS (
        SELECT
      ...
      FROM agg a, loss l;
    `;
    const params = [...f1.params, ...f2.params];
    const { rows } = await pool.query(sql, params);
    res.json(rows[0] || {});
  } catch (err) {
    console.error('production-report/kpis error:', err);
...
/* ================================================================== */
router.get('/written-premium-by-lob', async (req, res) => {
  const { from, to } = req.query;
  const f = bookMonthFilter(from, to);
  try {
    const sql = `
      SELECT
...
/* ================================================================== */
router.get('/inforce-distribution', async (req, res) => {
  const { from, to } = req.query;
  const f = bookMonthFilter(from, to);
  try {
    const sql = `
...
/* ================================================================== */
router.get('/nb-vs-renewal-trend', async (req, res) => {
  const { from, to } = req.query;
  const f = bookMonthFilter(from, to);
  try {
    const sql = `
...
/* ================================================================== */
router.get('/loss-ratio-by-lob', async (req, res) => {
  const { from, to } = req.query;
  const f1 = bookMonthFilter(from, to, 'bm', 1);
  const f2 = bookMonthFilter(from, to, 'bm', f1.nextIdx);
  try {
...
/* ================================================================== */
router.get('/policy-movement', async (req, res) => {
  const { from, to } = req.query;
  const f = bookMonthFilter(from, to);
  try {
    const sql = `
