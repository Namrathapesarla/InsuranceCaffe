...
/* ================================================================== */
router.get('/risk-by-lob', async (req, res) => {
  const { from, to } = req.query;
  const f1 = bookMonthFilter(from, to, 'bm', 1);
  const f2 = bookMonthFilter(from, to, 'bm', f1.nextIdx);
  try {
...
/* ================================================================== */
router.get('/risk-by-region', async (req, res) => {
  const { from, to } = req.query;
  const f1 = bookMonthFilter(from, to, 'bm', 1);
  const f2 = bookMonthFilter(from, to, 'bm', f1.nextIdx);
  try {
...
/* ================================================================== */
router.get('/uw-decisions', async (req, res) => {
  const { from, to } = req.query;
  const f = bookMonthFilter(from, to);
  try {
    const sql = `
      SELECT
        COALESCE(fpt.event_type_code, 'UNKNOWN') AS decision,
