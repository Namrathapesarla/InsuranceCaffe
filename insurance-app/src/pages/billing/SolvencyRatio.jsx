...
export default function SolvencyRatio() {
  ...
  return(<div>
    <PageHeader title={reportingUS?"Solvency Ratio Monitor":"Solvency Ratio Monitor (≥150%)"} subtitle={reportingUS?undefined:"Quarterly solvency ratio monitoring per IRDAI mandate with auto-alerts when approaching threshold"} breadcrumbs={[{label:'Billing',path:'/billing/invoices'},{label:'Solvency'}]}/>
    ...
    {!reportingUS&&Number(kpis.solvency_proxy)<150&&<div style={{padding:'1rem',background:'#fef2f2',border:'1px solid #fecaca',borderRadius:'0.5rem',color:'#dc2626',marginBottom:'1.5rem',fontSize:'0.85rem'}}><strong>ALERT:</strong> Solvency proxy ratio ({kpis.solvency_proxy}%) is below IRDAI minimum of 150%. Immediate action required.</div>}
